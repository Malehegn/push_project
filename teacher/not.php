...
<?php echo "<img src='../01-for login page/images/photos/".$test['upload']."' width='210' height='210 alt='user photo'/>"; ?>	
		 -->
							...
							<legend><h2 style="color:darkred">Notifications</h2></legend>
										<ul>		<?php
	$con=@mysql_connect ('localhost','root','');
mysql_select_db("onlinegradeinquiry",$con);
	$result = mysql_query("SELECT * FROM notification WHERE position='teacher' ORDER BY id DESC");
			while($row = mysql_fetch_array($result))
				{
					echo "<b>".'<li style=color:darkblue>'.$row['message'].'</li>'."</b>";
				}
	?>
	</ul>
							</fieldset>
